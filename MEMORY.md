...
- **2026-03-13 (VQ Consolidation for RFQ 1130350)**: Created consolidated VQ file from 03-10/11/12 outputs for RFQ 1130350 (136 lines). **Fixes applied:** (1) BP ID→Search Key mapping for 30 lines, (2) COO: converted ISO codes→full country names (iDempiere expects "China" not "CN"), cleared 1 false positive (EXW Israel is shipping terms, not COO), (3) Stripped vendor names from Vendor Notes, (4) RoHS→Y/N only. **Doc fix:** Updated vq-loading.md COO field to specify full names + added reference table. **Roadmap:** Added C8 MFR Text Validation. **Output:** `rfq_sourcing/vq_loading/output/consolidated-coo-fixed.csv`.
