...
- **2026-03-13 (Shared CSV Utilities)**: Fixed CSV parsing bug in ad-hoc inventory analysis — naive `split(',')` returned 8 rows for W111 instead of 105 due to quoted description fields containing commas. **Created:** `shared/csv-utils.js` with proper CSV parsing (handles quoted fields, escaped quotes). **API:** `readCSVFile()` returns object with `filterByColumn()`, `sumColumn()`, `colIndex()` helpers. **W111 corrected:** 105 lines, 94 unique items, **$81,051.04** total value (not $8,130.83). Updated CLAUDE.md with Shared Utilities section mandating use of csv-utils for all CSV parsing. Commits: 85e6dfa, 5ab98a5.
