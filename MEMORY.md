...
- **2026-03-13 (VQ Consolidation for RFQ 1130350)**: Created consolidated VQ file from 03-10/11/12 outputs for RFQ 1130350 (136 lines). **Fixes applied:** (1) BP ID→Search Key mapping for 30 lines (old files used c_bpartner_id instead of search_key - now fixed in vq-parser field-mapper.js:303-304), (2) COO cleanup: 8 country names→ISO codes + 10 garbage values cleared, (3) Stripped 7 vendor names from Vendor Notes. **Root cause:** Old extractions predated the search_key fix. **Output:** `rfq_sourcing/vq_loading/output/consolidated-all-fixed.csv`. Commits: f43527b, 27af421, 9646165.
