#!/usr/bin/env node
/**
 * Audit which IMAP folders exist in excess@orangetsunami.com vs which ones
 * the excess workflow handler routes messages to.
 *
 ...
 *
 * Read-only. Lists folders + checks for the five folders the handler uses:
 *   Processed, NeedsPartner, NeedsReview, NotOffer, LargeOfferApprovals
 */
...
(async () => {
  ...
  console.log(`Connected to excess@orangetsunami.com`);
  console.log('');
  ...
  if (allPresent) {
    ...
    console.log('Some folders missing. Run create-excess-folders.js next to create them.');
  }
