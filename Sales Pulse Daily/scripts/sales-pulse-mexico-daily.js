...
function execQuery(sql) {
  try {
    ...
    // UNIQUE FILENAME to prevent race condition with USA/VP briefs running simultaneously
...
async function collectData() {
  ...
  // Calculate totals from rep data
  data.repTotals = {
    rfq_lines: data.repActivity.reduce((sum, r) => sum + parseInt(r.rfq_lines || 0), 0),
    cq_lines: data.repActivity.reduce((sum, r) => sum + parseInt(r.cq_lines || 0), 0),
    cq_sold: data.repActivity.reduce((sum, r) => sum + parseInt(r.cq_sold || 0), 0),
    so_lines: data.repActivity.reduce((sum, r) => sum + parseInt(r.so_lines || 0), 0),
    so_revenue: data.repActivity.reduce((sum, r) => sum + parseFloat(r.so_revenue || 0), 0),
    so_gp: data.repActivity.reduce((sum, r) => sum + parseFloat(r.so_gp || 0), 0)
  };
...
/**
 * Section 2.2A: Top 10 Late SO Lines (No Revenue Filter)
 */
...
function getRepActivity(queryFile) {
  ...
  return parseRows(execQuery(sqlMatch[0]), [
    'sales_rep_name', 'rfq_lines', 'cq_lines', 'cq_sold', 'so_lines', 'so_revenue', 'so_gp'
  ]);
}
...
function generateSection2(data) {
  ...
  // 2.2A Top 10 Late SO Lines (No Revenue Filter)
  html += '<div class="subsection-header">Top 10 Late SO Lines (3-31 days past due)</div>';
  html += '<div style="font-size: 11px; color: #666; margin-bottom: 8px;">🟡 Yellow: 3-7 days late | 🔴 Red: 8+ days late | Rolling 31-day window | Top 10 by revenue (no minimum) | Unshipped amounts shown</div>';
...
function generateSection3(data) {
  ...
  if (data.repActivity.length > 0) {
    ...
    data.repActivity.forEach(rep => {
      const rfqStyle = (!rep.rfq_lines || rep.rfq_lines === '0' || rep.rfq_lines === 0) ? 'color: #d32f2f;' : '';
      const cqStyle = (!rep.cq_lines || rep.cq_lines === '0' || rep.cq_lines === 0) ? 'color: #d32f2f;' : '';
      const soldStyle = (!rep.cq_sold || rep.cq_sold === '0' || rep.cq_sold === 0) ? 'color: #d32f2f;' : '';
      const soStyle = (!rep.so_lines || rep.so_lines === '0' || rep.so_lines === 0) ? 'color: #d32f2f;' : '';
      const revStyle = (!rep.so_revenue || rep.so_revenue === '0' || rep.so_revenue === 0 || parseFloat(rep.so_revenue) === 0) ? 'color: #d32f2f;' : '';
      const gpStyle = (!rep.so_gp || rep.so_gp === '0' || rep.so_gp === 0 || parseFloat(rep.so_gp) === 0) ? 'color: #d32f2f;' : '';
      html += `
        <tr>
          <td><strong>${rep.sales_rep_name}</strong></td>
          <td style="text-align: center; ${rfqStyle}">${formatNumber(rep.rfq_lines)}</td>
          <td style="text-align: center; ${cqStyle}">${formatNumber(rep.cq_lines)}</td>
          <td style="text-align: center; ${soldStyle}">${formatNumber(rep.cq_sold)}</td>
          <td style="text-align: center; ${soStyle}">${formatNumber(rep.so_lines)}</td>
          <td class="number" style="${revStyle}">${formatCurrency(rep.so_revenue)}</td>
          <td class="number" style="${gpStyle}">${formatCurrency(rep.so_gp)}</td>
        </tr>`;
    });

    // Add TOTAL row
    const totalRfqStyle = (!data.repTotals.rfq_lines || data.repTotals.rfq_lines === 0) ? 'color: #d32f2f;' : '';
    const totalCqStyle = (!data.repTotals.cq_lines || data.repTotals.cq_lines === 0) ? 'color: #d32f2f;' : '';
    const totalSoldStyle = (!data.repTotals.cq_sold || data.repTotals.cq_sold === 0) ? 'color: #d32f2f;' : '';
    const totalSoStyle = (!data.repTotals.so_lines || data.repTotals.so_lines === 0) ? 'color: #d32f2f;' : '';
    const totalRevStyle = (!data.repTotals.so_revenue || data.repTotals.so_revenue === 0) ? 'color: #d32f2f;' : '';
    const totalGpStyle = (!data.repTotals.so_gp || data.repTotals.so_gp === 0) ? 'color: #d32f2f;' : '';
    html += `
      <tr style="border-top: 2px solid #333; background-color: #f5f5f5; font-weight: bold;">
        <td><strong>TOTAL</strong></td>
        <td style="text-align: center; ${totalRfqStyle}">${formatNumber(data.repTotals.rfq_lines)}</td>
        <td style="text-align: center; ${totalCqStyle}">${formatNumber(data.repTotals.cq_lines)}</td>
        <td style="text-align: center; ${totalSoldStyle}">${formatNumber(data.repTotals.cq_sold)}</td>
        <td style="text-align: center; ${totalSoStyle}">${formatNumber(data.repTotals.so_lines)}</td>
        <td class="number" style="${totalRevStyle}"><strong>${formatCurrency(data.repTotals.so_revenue)}</strong></td>
        <td class="number" style="${totalGpStyle}"><strong>${formatCurrency(data.repTotals.so_gp)}</strong></td>
      </tr>`;
...
function generateFooter() {
  ...
  <div class="footer">
    ...
    <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #ddd;">
      <em>Note: Criteria and thresholds are subject to refinement based on feedback.</em><br>
...
async function main() {
  ...
  console.log(`    - RFQ Lines: ${formatNumber(data.repTotals.rfq_lines)}`);
  console.log(`    - CQ Lines: ${formatNumber(data.repTotals.cq_lines)} (${formatNumber(data.repTotals.cq_sold)} sold)`);
  console.log(`    - SO Lines: ${formatNumber(data.repTotals.so_lines)}`);
  console.log();
  console.log('='.repeat(60));
}
