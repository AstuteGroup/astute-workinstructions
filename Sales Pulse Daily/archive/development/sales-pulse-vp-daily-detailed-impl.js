...
async function collectData() {
  ...
  // SECTION 2: Needs Attention
  console.log('\nSection 2: Needs Attention');
  console.log('  - Fetching late shipments...');
  data.lateShipments = getLateShipments(queryFile);

  console.log('  - Fetching past due summary...');
  data.pastDueSummary = getPastDueSummary(queryFile);
  ...
  console.log('\nSection 3: Yesterday\'s Activity by Region');
  console.log('  - Fetching global activity...');
  data.globalActivity = getGlobalActivity(queryFile);
...
function getTop5Orders(queryFile) {
  ...
  const sqlMatch = top5Query.match(/SELECT[\s\S]+?LIMIT 5;/);
  if (!sqlMatch) return [];
...
function getNewCustomersSold(queryFile) {
  ...
  const sqlMatch = newCustomersQuery.match(/SELECT[\s\S]+?ORDER BY total_revenue DESC;/);
  if (!sqlMatch) return [];
...
function getStrategicAccountsActivity(queryFile) {
  ...
  return parseRows(execQuery(sqlMatch[0]), [
    'account_name', 'ise_name', 'region', 'rfq_lines', 'cq_lines', 'cq_sold', 'so_count'
  ]);
}
...
/**
 * Section 2.2: Past Due Summary by Region
 */
function getPastDueSummary(queryFile) {
  const queries = fs.readFileSync(queryFile, 'utf8');
  const summaryQuery = queries.split('2.2 PAST DUE SUMMARY BY REGION')[1]
    .split('2.3 INSIDE SALES REPS ALERT')[0]
    .trim();

  const sqlMatch = summaryQuery.match(/SELECT[\s\S]+?ORDER BY region;/);
  if (!sqlMatch) return [];

  return parseRows(execQuery(sqlMatch[0]), [
    'region', 'so_lines_past_due', 'total_revenue_past_due'
  ]);
}
...
function getGlobalActivity(queryFile) {
  ...
  const sqlMatch = globalQuery.match(/SELECT[\s\S]+?as so_revenue;/);
  if (!sqlMatch) {
...
function getRegionalActivity(queryFile) {
  ...
  return parseRows(execQuery(sqlMatch[0]), [
    'region', 'manager', 'rfq_lines', 'cq_lines', 'cq_sold', 'so_lines'
  ]);
}
    ...
    data.strategicAccounts.forEach(acct => {
      html += `
        <tr>
          ...
          <td>${acct.ise_name}</td>
          <td>${acct.region}</td>
          <td style="text-align: center;">${formatNumber(acct.rfq_lines)}</td>
          <td style="text-align: center;">${formatNumber(acct.cq_lines)}</td>
          <td style="text-align: center;">${formatNumber(acct.cq_sold)}</td>
          <td style="text-align: center;">${formatNumber(acct.so_count)}</td>
        </tr>`;
    });
    ...
    data.reactivatedCustomers.forEach(cust => {
      html += `
        <tr>
          ...
          <td class="number">${cust.order_count}</td>
          ...
          <td><span class="badge badge-green">${formatNumber(cust.days_since_last_order)} days</span></td>
...
function generateSection2(data) {
  ...
  // 2.1 Late Shipments
  html += '<div class="subsection-header">Late Shipments (3+ Days Past Promise Date)</div>';
  if (data.lateShipments.length > 0) {
    html += `<div class="alert-box red">
      <strong>${data.lateShipments.length} orders are late</strong> — Total value: ${formatCurrency(data.lateShipments.reduce((sum, s) => sum + parseFloat(s.total_revenue || 0), 0))}
    </div>`;
    ...
    data.lateShipments.forEach(ship => {
      const badgeColor = ship.color_code === 'red' ? 'badge-red' : 'badge-yellow';
      html += `
        <tr>
          <td><strong>${ship.customer_name}</strong></td>
          <td style="font-size: 11px;">${ship.sales_order}</td>
          <td>${ship.ise_name}</td>
          <td>${ship.region}</td>
          <td class="number">${formatCurrency(ship.total_revenue)}</td>
          <td style="font-size: 10px;">${ship.part_numbers || 'N/A'}</td>
          <td>${ship.promise_date}</td>
          <td style="text-align: center;">${ship.in_stock}</td>
          <td style="text-align: center;"><span class="badge ${badgeColor}">${ship.days_late} days</span></td>
        </tr>`;
    });

    html += '</tbody></table>';
  } else {
    html += '<div class="alert-box green">✓ No late shipments ($250K+ / strategic / new customer)</div>';
  }

  // 2.2 Past Due Summary
  html += '<div class="subsection-header">Past Due Summary by Region</div>';
  if (data.pastDueSummary.length > 0) {
    html += `<table>
      <thead>
        <tr>
          <th>Region</th>
          <th style="text-align: right;">SO Lines Past Due</th>
          <th style="text-align: right;">Total Revenue</th>
        </tr>
      </thead>
      <tbody>`;

    data.pastDueSummary.forEach(region => {
      html += `
        <tr>
          <td><strong>${region.region}</strong></td>
          <td class="number">${formatNumber(region.so_lines_past_due)}</td>
          <td class="number">${formatCurrency(region.total_revenue_past_due)}</td>
        </tr>`;
    });

    html += '</tbody></table>';
  } else {
    html += '<div class="empty-state">No past due orders by region</div>';
  }
  ...
  if (data.iseAlerts.length > 0) {
    html += `<div class="alert-box yellow">
      <strong>${data.iseAlerts.length} ISEs</strong> have not loaded an RFQ in 3+ days
    </div>`;
    ...
    data.iseAlerts.forEach(ise => {
      ...
      html += `
        <tr>
          ...
          <td style="text-align: center;"><span class="badge ${badgeColor}">${ise.days_inactive} days</span></td>
        </tr>`;
    });
...
function generateSection3(data) {
  ...
  // 3.1 Global Activity
  html += '<div class="subsection-header">Global Activity Summary</div>';
  const ga = data.globalActivity;
  const closeRate = ga.cq_lines > 0 ? (parseInt(ga.cq_sold) / parseInt(ga.cq_lines)) : 0;

  html += `<div class="metrics">
    <div class="metric">
      <div class="metric-label">RFQ Lines</div>
      <div class="metric-value">${formatNumber(ga.rfq_lines)}</div>
      <div class="metric-trend">${formatNumber(ga.rfq_customers)} customers</div>
    </div>
    <div class="metric">
      <div class="metric-label">CQ Lines</div>
      <div class="metric-value">${formatNumber(ga.cq_lines)}</div>
      <div class="metric-trend">${formatNumber(ga.cq_sold)} sold (${formatPercent(closeRate)})</div>
    </div>
    <div class="metric">
      <div class="metric-label">Orders Booked</div>
      <div class="metric-value">${formatNumber(ga.so_lines)} lines</div>
      <div class="metric-trend">${formatCurrency(ga.so_revenue)}</div>
    </div>
  </div>`;

  // 3.2 Regional Breakdown
  html += '<div class="subsection-header">By Region</div>';
...
function generateFooter() {
  ...
  <div class="footer">
    ...
    <ul>
      ...
      <li><strong>Reactivated Customers:</strong> Customers with no orders in the past 6+ months</li>
    </ul>

    <strong>Section 2: Needs Attention</strong>
    <ul>
      <li><strong>Late Shipments:</strong> 3+ days past promise date AND ($250K+ revenue OR strategic account OR new customer first order)</li>
      <li><strong>Color Coding:</strong> Yellow = 3-5 days late, Red = 5+ days late</li>
      <li><strong>ISE Alerts:</strong> Inside sales reps with no RFQ loaded in 3+ days</li>
      <li><strong>Low Margin Trail:</strong> Orders <18% GM (Josh-approved audit trail)</li>
    </ul>

    <strong>Section 3: Yesterday's Activity</strong>
    <ul>
      <li><strong>Regions:</strong> USA (Jeff Wallace), MEX (Joel Marquez), APAC-Laurel (Laurel Kee), APAC-Silvia (Silvia Munoz), Other (Josh Pucci)</li>
    </ul>
...
async function main() {
  ...
  console.log(`    - Late Shipments: ${data.lateShipments.length}`);
  ...
  console.log(`    - RFQ Lines: ${formatNumber(data.globalActivity.rfq_lines)}`);
  console.log(`    - CQ Lines: ${formatNumber(data.globalActivity.cq_lines)} (${formatNumber(data.globalActivity.cq_sold)} sold)`);
  console.log(`    - SO Lines: ${formatNumber(data.globalActivity.so_lines)} (${formatCurrency(data.globalActivity.so_revenue)})`);
  console.log();
  console.log('='.repeat(60));
}
