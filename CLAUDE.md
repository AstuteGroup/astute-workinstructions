...
> 24. **Market Intelligence** - Two complementary workflows: (1) **Market Profiling** scrapes NC for broker availability without sending RFQs, rotates through inventory continuously (500 MPNs/day), loads $0 availability VQs. (2) **Active Sourcing** selects 200 priority MPNs (top-requested + high-value MFRs), excludes from NC upload during price-check, sends real RFQs Mon/Thu. De-listing hides inventory from competitors. See `Trading Analysis/Market Profiling/market-profiling.md`.
