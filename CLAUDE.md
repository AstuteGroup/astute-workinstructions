...
> 24. **Market Intelligence** - Two complementary workflows: (1) **Market Profiling** runs autonomously 24/7 (every 30min), scrapes NC for broker availability without sending RFQs, self-regulating batch size (~50/tick, ~2,400/day), loads $0 availability VQs. (2) **Active Sourcing** selects 200 priority MPNs (top-requested + high-value MFRs), excludes from NC upload during price-check, sends real RFQs Mon/Thu. De-listing hides inventory from competitors. See `Trading Analysis/Market Profiling/market-profiling.md`.
