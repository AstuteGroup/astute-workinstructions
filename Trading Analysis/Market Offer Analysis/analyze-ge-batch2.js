...
const OFFER_ID = 1026032;
const OFFER_SEARCH_KEY = '1025925';
...
const SOURCE_SHEET = 'BOM';
const BATCH_LABEL = 'Batch 2 (BOM)';
...
async function main() {
  ...
  for (let bs = 0; bs < lines.length; bs += BATCH_SIZE) {
    ...
    const batchResults = await Promise.all(batch.map(async (line, j) => {
      ...
      // Price Advantage scoring is not yet implemented for consignment lots —
      // the GE "Unit Price" field is their book value (cost basis), not their
      // ask price, so the standard offer-vs-franchise comparison doesn't apply
      // cleanly. priceScore stays 0 and we note prices in the per-line detail
      // for human review.
      const priceScore = 0;
      const demandScore = scoreDemand(demand);
      const rawScore = supplyScore == null ? null : supplyScore + priceScore + demandScore;
      const maxScore = 65; // Supply 40 + Demand 25 (Price 35 not scored)
  ...
  if (csvBuffer) {
    ...
    console.log('  sidecar csv buffer: (none for this batch)');
  }
...
function buildLotSummaryRows(enriched, stats) {
  ...
  return [
    ['GE Aerospace Rev-Share Batch 2 (BOM) — Lot Analysis'],
    [],
    ...
    ['Volume'],
    ['BOM rows total',              1028],
    ['Sub-component rows (no MPN)', 675],
    ['Unique MPNs after dedupe',    256],
    ['Loaded into OT',              total],
    [],
...
function buildSidecarCsvBuffer() {
  // Batch 2 has no #N/A / #REF! errors like Batch 1's "Legacy Obsolete" sheet.
  // The BOM sheet's data quality issues (235 distributor-MFR filterings,
  // 675 sub-component rows without MPNs, 3 MPNs with E&O qty summed across
  // assemblies) are surfaced in the loader stats and the xlsx Lot Summary
  // sheet. No separate sidecar needed.
  return null;
}
...
function buildHtml(enriched, stats) {
  ...
  // Total qty + book value across the loaded set
  const totalQty   = enriched.reduce((s, e) => s + (e.qty || 0), 0);
  const totalValue = enriched.reduce((s, e) => s + ((e.qty || 0) * (e.price || 0)), 0);

  return `<html><body style="font-family:Segoe UI,Arial,sans-serif;font-size:13px;color:#222;max-width:820px">
<h2 style="margin-bottom:4px">GE Aerospace Rev-Share Batch 2 (BOM) — Analysis</h2>
<p style="color:#666;margin-top:0">
Source email: 4/8/2026 — Pat Bade @ GE Aerospace, "Items for rev share" (second list)<br>
OT offer search key: <b>${OFFER_SEARCH_KEY}</b> (chuboe_offer_id ${OFFER_ID})<br>
Source file: <code>Astute Rev Share Part 2.xlsx</code> (BOM sheet, 1028 rows)<br>
Companion to Batch 1 (the "Legacy Obsolete" list, search key 1025923) — same partner, richer field data.
</p>
...
<table style="border-collapse:collapse;margin:8px 0;font-size:12px">
  ...
  <tr>${cell('BOM data rows total')}${cell('1,028', 'right')}</tr>
  <tr>${cell('Sub-component rows (no MPN — skipped)')}${cell('675', 'right')}</tr>
  <tr>${cell('Unique MPNs after dedupe')}${cell(total, 'right')}</tr>
  <tr>${cell('Total qty across loaded lines')}${cell(totalQty.toLocaleString(), 'right')}</tr>
  <tr>${cell('<b>Total book value (qty × unit price)</b>')}${cell('<b>$' + totalValue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) + '</b>', 'right')}</tr>
</table>

<p style="color:#666;font-size:12px"><b>Note on MFR field cleanup:</b> 235 of the BOM rows had distributors (Arrow, Avnet, TTI, Future, etc.) in the <code>MFG'er</code> column — those were filtered to empty at extraction time so we don't corrupt OT with distributor-as-mfr. After filtering, 91 of 256 unique MPNs (35.5%) have a real manufacturer name.</p>
...
<h3>Attachments</h3>
<ul>
  <li><b>GE_Aerospace_RevShare_Batch2_BOM.xlsx</b> — 4 sheets: Lot Summary, Top 25 Opportunities, GE Internal — Push Back, Per-Line Detail (all ${total} sorted by score)</li>
</ul>

<p style="color:#666;font-size:11px;margin-top:20px">Same Market Offer Analysis pipeline as Batch 1: three-state franchise classification (IN_STOCK / FRANCHISE_OUT_OF_STOCK / NO_LISTING_*), <code>shared/mpn-classifier.js</code> for the NO_LISTING sub-classification, in-memory xlsx buffer (no disk writes), bulk demand fetch via <code>shared/market-data.getBulkMarketData</code>. Batch 2 differs from Batch 1 in that we now have actual MFR text + price + CPC fields populated, so franchise hit rate and demand-side scoring should be much more meaningful.</p>
</body></html>`;
}
...
async function sendEmail(htmlBody, xlsxBuffer, csvBuffer) {
  ...
  const attachments = [
    { filename: 'GE_Aerospace_RevShare_Batch2_BOM.xlsx', content: xlsxBuffer },
  ];
  if (csvBuffer) {
    attachments.push({ filename: 'GE_Aerospace_RevShare_Batch2_source_quality_issues.csv', content: csvBuffer });
  }
  return transporter.sendMail({
    from: `"Market Offer Analysis" <${FROM_EMAIL}>`,
    to: TO_EMAIL,
    subject: 'GE Aerospace Rev-Share Batch 2 (BOM) — 256 MPNs, $721K book value',
    html: htmlBody,
    attachments,
  });
}
