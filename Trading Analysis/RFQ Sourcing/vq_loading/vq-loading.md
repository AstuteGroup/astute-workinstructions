...
**Escalations are INTERNAL-ONLY (operator directive 2026-05-26).** The four escalation actions — `need_info_vendor`, `clarify_vendor`, `needs_vendor`, `needs_review` — and the partial-load `clarifications[]` path all send **one email** to the internal owners: operator (Jake) + the internal forwarder (envelope From, if `@astutegroup.com`) + the resolved buyer (`buyerId`) + any `@astutegroup.com` address already on the Cc. The **external broker is never emailed** — they're named on the body ("external sender — not emailed") so the operator can loop them in manually. Reply-To is `vq@orangetsunami.com` so an internal reply round-trips through the sidecar-stitch path.

This **retires the earlier broker-outreach override**. Rationale: VQ loading is internal data capture; the buyer/sourcer or support forwarder is the internal owner who chases the broker, not the loader. It also supersedes the two-email split (`41b6362`), which sent the forwarder a separate copy the operator couldn't see (UID 8684 read as "forwarder skipped"). With internal-only routing the forwarder is simply a visible recipient on the one email.

**Partial-load with consolidated clarifications.** Multi-vendor inbound emails go through a partial-load pattern: the agent loads the clean vendors via `load_vq` AND passes a `clarifications[]` array describing per-vendor outstanding asks for the problematic ones. The handler writes the clean quotes, then fires ONE consolidated internal email (operator + forwarder + buyer) covering all the loose ends. A sidecar of kind `partial_clarify` persists outstanding state so the next agent tick stitches the reply.
