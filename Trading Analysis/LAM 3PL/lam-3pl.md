...
*Updated: 2026-08-06* — **Receipt date staleness fix.** Fixed two bugs that caused old POVs to incorrectly show as PENDING RECEIPT even when parts had been received: (1) `aggregateInventory()` was not preserving `latestReceiptDate` from per-warehouse inventory objects, so the receipt-date check in `isPovStale()` always got null. (2) Excel serial dates from cached inventory (e.g., 45509) were being compared as strings against ISO dates (2025-04-14), causing incorrect filtering. Added `toDateString()` conversion in both the inventory parser and cache loader. Example: CPC 662-094083-001 with POV0070042 (2025-04-14) now correctly shows as MEDIUM priority instead of PENDING RECEIPT because the inventory receipt date (2024-08-05) proves parts were received.
*Updated: 2026-08-06* — **Escalations tab always present.** The Escalations tab is now always created in the Excel output even when empty (no new escalations that week). Previously the tab was conditionally created only when `newEntries.length > 0`. This ensures consistent tab structure across all weekly outputs.
*Updated: 2026-08-06* — **Stale POV data cleared from output.** When a POV is filtered as stale (receipt date > PO date), the POV-related columns (Recent POV, On Order Qty, OT PO, Tracking, Last Promise Date, PO Created Date, Last Updated) are now cleared. Previously, the staleness filter only changed the priority from PENDING RECEIPT to the shortfall-based priority but left all the old POV data in columns N-T. Now `buildAlert()` receives the filtered POV for display (empty if stale) while still using raw Purchased MPN for NOT ON AVL validation.
*Updated: 2026-08-06* — **W118 consignment inventory.** W118 (LAM Consignment) is now included in inventory counts alongside W111 and W115. W118 stock counts toward total inventory (preventing parts from showing as CRITICAL when consignment is available), but still needs monthly enrichment since we control pricing. Added "W118 Consignment" column showing qty. Consignment is technically LAM-owned stock that we buy from LAM to sell to LAM.
*Updated: 2026-08-06* — **W118 margin compression flagging.** When sourcing runs on a part with W118 consignment stock (W118 Qty > 0), the sourcing script now checks if franchise pricing results in <18% margin. If so, the "W118 Margin Compression" column is set to "YES" (with red highlight in Excel). This flags parts where buying from franchise would compress margins below the target threshold, allowing escalation to LAM for price approval before purchasing.
*Updated: 2026-08-06* — **Warehouse MPN Audit.** Added weekly audit that compares warehouse inventory MPNs (W111 + W115 only, NOT W118 consignment) against the roster/AVL. Detects: (1) **Packaging variants** — inventory MPNs that appear to be alternate packaging of AVL MPNs (e.g., `BK-ABC-12-R` vs `ABC-12`), flagged as candidates for confirmation; (2) **Unmatched MPNs** — inventory with no AVL match, needs investigation (mislabel? wrong warehouse? obsolete?). Results appear in the "Warehouse MPN Audit" tab with amber/red highlighting. Common suffixes stripped for matching: `-R`, `/R`, `-TR`, `-BK`, `BK-`, `-CT`, `-TRAY`, `-ND`, `-DKR`, `-E`, `-E3`. W118 is excluded because it's LAM consignment stock from other programs.
*Updated: 2026-08-06* — **Email recipient fix.** Manual runs now correctly send to Jake only. Josh is only included on automated cron runs (via NOTIFY_EMAIL env var). Fixed default recipient logic that was sending to both for all runs.
*Updated: 2026-08-06* — **Warehouse audit placeholder bug fix.** Fixed bug where MPNs containing "NA" substring (e.g., `INA188IDR`, `INA826AIDGKR`) were incorrectly treated as placeholder MPNs and excluded from AVL matching. The `isPlaceholderMPN()` function was using `.includes()` to match "NA" (a placeholder), but this also matched legitimate MPNs. Changed to exact match only. This reduced false unmatched count by 4 items.
*Updated: 2026-08-06* — **Warehouse audit Complete AVL fix.** The audit was only checking MPNs against the Master Roster, missing alternate MPNs in the Complete AVL (e.g., `CRCW08057K50JNEA` is in AVL for CPC 615-901294-752 but roster only has `RC0805JR-077K5L`). Now loads both roster AND Complete AVL for matching. Reduced unmatched from 15 → 1.
*Updated: 2026-08-06* — **Warehouse audit leading # fix.** Added stripping of leading `#` character in MPN normalization. LAM uses `#ABC-12` format but warehouse labels as `BK-ABC-12-R`. Now both normalize to `ABC-12` and match as packaging variant.
