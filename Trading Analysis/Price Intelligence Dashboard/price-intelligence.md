...
### Step 1 — Confirm the MPN with the operator

Echo back the MPN you parsed and ask whether they want **exact** match (default) or **loose** prefix match (catches sister variants like the TR / different revision suffix):

> "Building Price Intelligence Dashboard for `<MPN>`. Default: exact `chuboe_mpn_clean` match (catches formatting variants only). Want `--loose` (prefix match, catches TR / revision variants)?"
...
| Flag | Default | Effect |
|------|---------|--------|
| `--mpn "<MPN>"` | required | The part to analyze. Quote it — colons, spaces, and dashes are common. |
...
| `--email` | off | Also email the dashboard via `shared/notifier.js` to `OPERATOR_EMAIL` (default Jake). |
...
- **Header** — MPN, date range, row counts.
...
- **Footer** — match strategy and generation timestamp.
...
| `output/<MPN>_<date>.html` | Generated dashboards (gitignored except `.gitkeep`) |
