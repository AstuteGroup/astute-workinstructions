...
function buildSection2HTML(regionalData) {
  ...
  const subregionRows = apacData.map(r => `
    <tr class="subregion-row">
      <td>${r.region === 'APAC-Laurel' ? '↳ Laurel (SGP, 3 sellers)' :
            r.region === 'APAC-Kris' ? '↳ Kris/Silvia (PHL/China, 6)' :
            r.region === 'APAC-Lavanya' ? '↳ Lavanya (IND, 3 sellers)' : r.region}</td>
      <td>${r.rfq_lines}</td>
  ...
  return `
    ...
    <div class="section">
      ...
      <table>
        ...
        <tbody>
          <tr class="region-row">
            <td><strong>USA</strong> (Jeff, 8 sellers)</td>
          ...
          <tr class="region-row">
            <td><strong>MEX</strong> (Joel, 8 sellers)</td>
          ...
          <tr class="region-row">
            <td><strong>APAC TOTAL</strong> (12 sellers)</td>
...
async function collectSection3Metrics() {
  ...
  const winsQuery = `
    WITH wins AS (
      SELECT
        ...
        bp.name AS customer,
        u.name AS seller,
        COALESCE(p.name, 'N/A') AS mpn,
        'TBD - buyer lookup' AS buyer,
    ...
    SELECT
      ...
      customer,
      seller,
      STRING_AGG(DISTINCT mpn, ', ') AS parts,
      STRING_AGG(DISTINCT buyer, ', ') AS buyers,
  ...
  const winsData = parseRows(execQuery(winsQuery), ['region', 'customer', 'seller', 'parts', 'buyers', 'lines', 'amount']);
  ...
  const buildRegionWins = (wins, regionName) => {
    ...
    const items = wins.map(w =>
      `• ${w.customer} — ${w.lines} lines, ${formatCurrency(w.amount)}<br>
        &nbsp;&nbsp;Parts: ${w.parts || 'N/A'}<br>
        &nbsp;&nbsp;Seller: ${w.seller} | Buyers: ${w.buyers || 'N/A'}`
    ).join('<br>\n        ');
  ...
  return `
    ...
    <div class="section">
      ...
      <div class="alert-box opportunity">
        <div class="alert-title">💎 HIGH-PROBABILITY CUSTOMERS (30-50% win rate, quoted in last 5 days) — Top 5</div>
...
function buildSection5HTML(wtdData) {
  ...
  const rows = wtdData.map(r => `
    <tr class="${r.region.startsWith('APAC') ? 'subregion-row' : 'region-row'}">
      <td>${r.region === 'APAC-Laurel' ? '↳ Laurel (SGP, 3 sellers)' :
            r.region === 'APAC-Kris' ? '↳ Kris/Silvia (PHL/China, 6)' :
            r.region === 'APAC-Lavanya' ? '↳ Lavanya (IND, 3 sellers)' :
            r.region === 'USA' ? '<strong>USA</strong> (Jeff, 8 sellers)' :
            r.region === 'MEX' ? '<strong>MEX</strong> (Joel, 8 sellers)' : r.region}</td>
...
async function collectSection6Metrics() {
  ...
  // Trending Manufacturers (most RFQ lines in last 5 business days)
  ...
  // Trending Parts (most RFQ lines in last 5 business days)
  ...
  const trendingManufacturers = parseRows(execQuery(trendingMfrQuery),
    ['manufacturer', 'rfq_count', 'quoted_count', 'sold_count']);
  const trendingParts = parseRows(execQuery(trendingPartsQuery),
    ['mpn', 'manufacturer', 'rfq_count', 'quoted_count', 'sold_count']);
...
function buildSection6HTML(marketData) {
  if (!marketData.trendingManufacturers || marketData.trendingManufacturers.length === 0) {
    return `
      <!-- MARKET PULSE -->
      <div class="section">
        <div class="section-title">📈 Market Pulse (Last 5 Business Days)</div>
        <div class="footnote">No trending activity in last 5 days</div>
      </div>
    `;
  }
      ...
      <table style="background: white;">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>RFQ line MPNs from last 5 business days</td>
