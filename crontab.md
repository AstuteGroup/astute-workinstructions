...
|---|---|---|---|---|
| `0 10 * * 1` | **Inventory Fetch & Parse** | Weekly Mon 10:00 UTC (5 AM CT) — fetches Infor xlsx from email, parses ALL warehouses, caches JSON for subordinate workflows | `shared/inventory-fetch-and-parse.js` | `/tmp/inventory-fetch-and-parse.log` |
| `15 11 * * 1` | **LAM Inventory** | Weekly Mon 11:15 UTC — LAM warehouses (W115, W118) → OT offers + threshold check + wrong warehouse check | `workflows/lam-inventory.js` | `/tmp/lam-inventory.log` |
| `30 11 * * 1` | **Free Stock Inventory** | Weekly Mon 11:30 UTC — Free Stock warehouses (W102, W104, W108, W109, etc.) → OT offers | `workflows/free-stock-inventory.js` | `/tmp/free-stock-inventory.log` |
| `45 11 * * 1` | **Consignment Inventory** | Weekly Mon 11:45 UTC — Consignment warehouses (W103, W106, W107, W117) → OT offers (prices blanked) | `workflows/consignment-inventory.js` | `/tmp/consignment-inventory.log` |
| `0 12 * * 1` | **LAM 3PL Runner** | Weekly Mon 12:00 — runs LAM 3PL reorder analysis (BOM → demand → franchise sourcing → RFQ writes → customer offer refresh). Uses cache. | `Trading Analysis/LAM 3PL/lam-kitting-runner.js` | `Trading Analysis/LAM 3PL/data/cron.log` |
