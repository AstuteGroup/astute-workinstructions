...
**IMAGE ATTACHMENTS (screenshot shipping confirmations):**
...
Then the tracking data may be in an image (common for TI, Mouser, DigiKey portal screenshots):
...
3. Extract tracking numbers, carrier, ship date, and MPNs from the image
4. Proceed with the normal routing flow using the extracted data

**Important:** Screenshots often contain MPN-to-tracking mappings (e.g., TI shows which parts shipped with which tracking). For multi-line orders, extract BOTH the tracking numbers AND the MPNs from the image to populate the `mpn` field correctly.
