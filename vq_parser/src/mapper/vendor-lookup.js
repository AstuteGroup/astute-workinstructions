...
/**
 * Extract company name hints from email domain
 * e.g., sales@greenchips.com → "greenchips", "green chips"
 */
function extractDomainHints(email) {
  ...
  const genericDomains = ['gmail', 'yahoo', 'hotmail', 'outlook', 'aol', 'mail', 'email', 'noreply'];
  if (genericDomains.includes(domain)) return [];

  const hints = [domain];

  // Add space-separated version (e.g., "greenchips" → "green chips")
  // Common patterns: camelCase, concatenated words
  const spaced = domain
    .replace(/([a-z])([A-Z])/g, '$1 $2')  // camelCase
    .replace(/(\d+)/g, ' $1 ')            // numbers
    .replace(/electronics?|intl|international|group|corp|inc|ltd|tech|usa|uk|eu/gi, ' $& ')  // common suffixes
    .replace(/\s+/g, ' ')
    .trim();
  ...
  const stripped = domain.replace(/(elec|electronics?|intl|international|group|corp|inc|ltd|tech|usa|uk|eu)$/gi, '').trim();
  if (stripped && stripped !== domain && stripped.length >= 3) {
    hints.push(stripped);
  }
...
/**
 * Look up vendor by email domain (fuzzy match on domain name)
 */
function lookupVendorByDomainDB(email) {
  if (!email) return null;
  ...
  for (const hint of hints) {
    ...
    try {
      ...
      const sql = `
        SELECT c_bpartner_id, name, value as search_key
        FROM adempiere.c_bpartner
        WHERE LOWER(name) LIKE '%${cleanHint}%'
          AND isactive = 'Y'
          AND name NOT ILIKE 'USE %'
          AND isvendor = 'Y'
        ORDER BY
          CASE WHEN LOWER(name) LIKE '${cleanHint}%' THEN 0 ELSE 1 END,
          created DESC
        LIMIT 3
      `;
      ...
      if (result) {
        ...
        if (lines.length > 0) {
          ...
          logger.info(`Vendor resolved by domain (${hint}): ${name.trim()} (${id.trim()})`);
          return {
            c_bpartner_id: id.trim(),
            name: name.trim(),
            search_key: searchKey ? searchKey.trim() : id.trim()
      ...
      logger.debug(`Domain lookup failed for hint "${hint}": ${err.message}`);
    }
  }
...
async function resolveVendor(emailBody, senderEmail = '', senderName = '') {
  ...
  // Strategy 1: Exact email lookup in ad_user
  ...
  // Strategy 2: Domain-based lookup (e.g., velocityelec.com → Velocity Electronics)
  const domainResult = lookupVendorByDomainDB(senderEmail);
  if (domainResult) {
    cache[cacheKey] = domainResult;
    saveCache(cache);
    return domainResult;
  }

  // Strategy 3: Name lookup using extracted sender name (heuristic - no LLM needed)
  if (senderName && senderName.length > 2) {
    const nameResult = lookupVendorByNameDB(senderName);
    if (nameResult) {
      logger.info(`Vendor resolved by name: ${nameResult.name} (${nameResult.c_bpartner_id})`);
  ...
  // Strategy 4: LLM inference (if API key configured)
